...
console.log('drawer');
{
  ...
  const timing = (sel) => page.$eval(sel, (e) => {
    const s = getComputedStyle(e);
    return s.animationDuration + ' ' + s.animationTimingFunction;
  });
  const entering = await timing('.drawer-panel');
  ...
  ok(await timing('.drawer-panel') === entering, `exit matches the entrance timing (${entering})`);
